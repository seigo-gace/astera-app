...
check(
  'Result download uses authenticated Blob bridge',
  resultPage.includes("credentials: 'include'") && resultPage.includes('const blob = await response.blob()') && resultPage.includes('URL.createObjectURL(blob)'),
  'Result download must fetch authenticated bytes and create a Blob URL',
);
check('Result download names file', resultPage.includes('anchor.download = filename'), 'Result download file name missing');
