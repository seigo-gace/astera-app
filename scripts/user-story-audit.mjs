...
const sourceRequirements = [
  ...
  ['src/features/composer/NativeComposerPage.tsx', 'executionLock.current'],
  ['src/features/composer/NativeComposerPage.tsx', 'ASTERA_RESPONSE_SECTIONS_INCOMPLETE'],
  ['src/features/composer/NativeComposerPage.tsx', 'function normalizeResult'],
  ['src/features/composer/NativeComposerPage.tsx', 'FILE_UPLOAD_PIPELINE_NOT_CONNECTED'],
  ['src/features/composer/NativeComposerPage.tsx', 'ASTERA_INPUT_TOO_LARGE'],
  ['src/platform/api-client.ts', "headers['Idempotency-Key'] = requestId"],
  ['src/features/composer/NativeComposerPage.tsx', 'const PURPOSE_CHOICES'],
  ['src/features/composer/NativeComposerPage.tsx', "apiRequest('/api/projects')"],
  ['src/features/composer/NativeComposerPage.tsx', '/app/settings/storage-destinations'],
  ['functions/_auth.ts', 'expiresIn: 60 * 60 * 24 * 7'],
  ['functions/_auth.ts', 'updateAge: 60 * 60 * 24'],
  ['functions/_auth.ts', 'freshAge: 60 * 15'],
  ['functions/_account-projection.ts', "existing?.account_status === 'active'"],
  ...
  ['contabo/billing-service/src/system/handlers-storage-checkout.ts', 'STORAGE_PLAN_CAPACITY_EXCEEDED'],
  ...
  ['src/features/checkout/CheckoutPage.tsx', '/api/storage/catalog'],
  ['src/features/checkout/CheckoutPage.tsx', '/api/storage/checkout-intents'],
];
...
const report = {
  ...
  coverage: [
    ...
    'completed account persistence and legacy pending-password repair',
    ...
    'registered Project selection from current Composer context',
    'saved external Storage Settings navigation from current Composer context',
    'catalog-owned Credit products and Checkout URL trust',
    'buy-once Storage checkout, plan capacity gate, and payment-posted capacity',
    'owned non-current device session revocation',
