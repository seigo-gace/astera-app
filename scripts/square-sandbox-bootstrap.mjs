...
async function main() {
  ...
  if (!plan) {
    ...
    console.log(`CREATED subscription plan ${PLAN_NAME} id=${plan.id}`);
  } else {
    console.log(`REUSED subscription plan ${PLAN_NAME} id=${plan.id}`);
  }
  ...
  for (const spec of variants) {
    ...
    if (!variation) {
      ...
      console.log(`CREATED ${spec.name} id=${variation.id}`);
    } else {
      console.log(`REUSED ${spec.name} id=${variation.id}`);
    }
  ...
  writeFileSync('audit-results/square-sandbox-billing-map.json', `${JSON.stringify({
    environment: 'sandbox',
    subscription_plan_id: plan.id,
