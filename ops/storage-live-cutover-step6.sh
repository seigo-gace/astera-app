...
NEW_TGS_IMAGE="$(sudo docker image ls --format '{{.Repository}}:{{.Tag}}' | grep '^astera-tgserver-storage-live:' | head -1 || true)"
[ -n "$NEW_TGS_IMAGE" ] || { echo 'OVERALL=BLOCKED'; echo 'REASON=BUILT_TGS_IMAGE_NOT_FOUND'; exit 1; }
BUILD_STAMP="${NEW_TGS_IMAGE#astera-tgserver-storage-live:}"
NEW_APP_IMAGE="astera-app-api-storage-live:${BUILD_STAMP}"
sudo docker image inspect "$NEW_TGS_IMAGE" >/dev/null 2>&1 || { echo 'OVERALL=BLOCKED'; echo 'REASON=BUILT_TGS_IMAGE_INVALID'; exit 1; }
sudo docker image inspect "$NEW_APP_IMAGE" >/dev/null 2>&1 || { echo 'OVERALL=BLOCKED'; echo 'REASON=MATCHING_APP_IMAGE_NOT_FOUND'; exit 1; }
...
[ -f "$APP_NEW_COMPOSE" ] || { echo 'OVERALL=BLOCKED'; echo 'REASON=EXACT_RELEASE_COMPOSE_NOT_FOUND'; exit 1; }
[ -d "$TGS_REL" ] || { echo 'OVERALL=BLOCKED'; echo 'REASON=EXACT_TGS_RELEASE_NOT_FOUND'; exit 1; }

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="/home/admin1/backups/storage-backend-${STAMP}"
TGS_OVERRIDE="$REL/tgs-live-resume-${STAMP}.override.yml"
APP_OVERRIDE="$REL/app-live-resume-${STAMP}.override.yml"
TGS_RB_OVERRIDE="$REL/tgs-rollback-${STAMP}.override.yml"
APP_RB_OVERRIDE="$REL/app-rollback-${STAMP}.override.yml"
...
log '============================================================'
log ' ASTERA STORAGE BACKEND LIVE CUTOVER - RESUME FROM STEP 6'
log '============================================================'
...
sudo docker commit --pause=false tgserver-tgs-1 "$OLD_TGS_TAG" >/dev/null
sudo docker commit --pause=false astera-app-api "$OLD_APP_TAG" >/dev/null
sudo docker image inspect "$OLD_TGS_TAG" >/dev/null
sudo docker image inspect "$OLD_APP_TAG" >/dev/null
ARMED=1
log 'ROLLBACK_SNAPSHOT=PASS|SOURCE=RUNNING_CONTAINERS'
