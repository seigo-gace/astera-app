import { FunctionHttpError, type D1Database } from '../part/billing-env.js';
...
export type PlanMonthlyCreditsSource = {
  included_credits: number;
  billing_variants: Array<{ billing_cycle: string; included_credits: number; active: boolean }>;
};

export function monthlyIncludedCreditsForPlan(plan: PlanMonthlyCreditsSource): number {
  const monthly = plan.billing_variants.find((variant) => variant.billing_cycle === 'monthly' && variant.active);
  const fromVariant = Number(monthly?.included_credits ?? 0);
  if (fromVariant > 0) return fromVariant;
  return Number(plan.included_credits ?? 0);
}
