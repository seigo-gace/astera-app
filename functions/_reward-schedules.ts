...
export async function applyDueRewardSchedules(env: RewardProgramEnv, actor: AsteraActorProjection): Promise<number> {
  let applied = 0;
  ...
  for (let safety = 0; safety < 24; safety += 1) {
    const row = await env.ASTERA_DB.prepare(
      `SELECT id,amount,remaining_grants,grants_applied,cadence_months,next_grant_at,reference_type,reference_id
       ...
       WHERE tenant_id=?1 AND user_id=?2 AND status='active' AND next_grant_at IS NOT NULL AND next_grant_at<=?3
       ORDER BY next_grant_at ASC LIMIT 1`,
    ).bind(actor.profile.tenant_id, actor.user.id, new Date(nowMs).toISOString()).first<ScheduleRow>();
    if (!row) break;
