...
export async function loadActiveCatalog(
  db: D1Database,
): Promise<ActiveCommercialCatalog> {
  try {
    ...
    const plans = (planResult.results ?? []).map((row) => {
      ...
      const billingVariants = allVariants
        ...
        .map((variant) => ({
          ...
          price_label: yen(
            Number(variant.recurring_amount),
            variant.recurring_interval,
          ),
          square_plan_variation_id: variant.square_plan_variation_id,
          active: variant.active === 1,
        }));
      const monthly = billingVariants.find(
        (variant) => variant.billing_cycle === "monthly",
      );
      return {
        plan_id: row.plan_id,
        id: row.plan_id,
        ...
        recurring_amount:
          monthly?.recurring_amount ?? Number(row.recurring_amount),
        recurring_interval:
          monthly?.recurring_interval ?? row.recurring_interval,
        monthly_price_yen:
          monthly?.recurring_amount ?? Number(row.recurring_amount),
        price_label:
          monthly?.price_label ??
          yen(Number(row.recurring_amount), row.recurring_interval),
        included_credits: Number(row.included_credits),
        monthly_credits: Number(row.included_credits),
        monthly_credits_label: Number(row.included_credits).toLocaleString(
          "ja-JP",
        ),
        ...
        square_plan_variation_id:
          monthly?.square_plan_variation_id ?? row.square_plan_variation_id,
    ...
    if (/no such table|D1_ERROR/i.test(message)) {
      throw new FunctionHttpError(
        503,
        "COMMERCIAL_CATALOG_SCHEMA_NOT_READY",
        "Commercial Catalog用D1 Migrationが適用されていません。",
        message,
      );
    }
    throw new FunctionHttpError(
      500,
      "COMMERCIAL_CATALOG_READ_FAILED",
      "Commercial Catalogを読み取れませんでした。",
      message,
    );
  }
}
  ...
  try {
    ...
    if (/no such table|D1_ERROR/i.test(message)) {
      throw new FunctionHttpError(
        ...
        "Subscription用D1 Migrationが適用されていません。",
        message,
      );
    }
    throw error;
