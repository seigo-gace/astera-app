...
type CatalogVersionRow = { version: string; checksum: string; published_at: string | null };
type PlanRow = {
  plan_id: string; display_name: string; description: string; currency: 'JPY';
  recurring_amount: number; recurring_interval: 'month' | 'year' | 'none'; included_credits: number;
  entitlement_ids: string; recommended: number; active: number; square_plan_variation_id: string | null;
};
type BillingVariantRow = {
  plan_id: string; billing_cycle: 'monthly' | 'annual'; recurring_amount: number;
  recurring_interval: 'month' | 'year'; included_credits: number;
  square_plan_variation_id: string | null; active: number;
};
type CreditProductRow = {
  product_id: string; display_name: string; description: string; currency: 'JPY';
  amount: number; credits: number; active: number; square_catalog_object_id: string | null;
};
type SubscriptionRow = {
  id: string; tenant_id: string; catalog_version: string; plan_id: string; billing_cycle: 'monthly' | 'annual';
  provider_subscription_id: string | null; status: string; current_period_start: string | null;
  current_period_end: string | null; cancel_at_period_end: number;
};
...
export type ActiveCommercialCatalog = {
  catalog_version: string; checksum: string; published_at: string;
  plans: Array<{
    plan_id: string; id: string; display_name: string; name: string; description: string; currency: 'JPY';
    recurring_amount: number; recurring_interval: string; monthly_price_yen: number; price_label: string;
    included_credits: number; monthly_credits: number; monthly_credits_label: string;
    entitlement_ids: string[]; features: string[]; recommended: boolean; active: boolean;
    square_plan_variation_id: string | null;
    billing_variants: Array<{
      billing_cycle: BillingCycle; recurring_amount: number; recurring_interval: 'month' | 'year';
      included_credits: number; price_label: string; square_plan_variation_id: string | null; active: boolean;
    }>;
  }>;
  creditProducts: Array<{
    product_id: string; id: string; display_name: string; name: string; description: string; currency: 'JPY';
    amount: number; price_label: string; credits: number; credits_label: string; active: boolean;
...
export async function loadActiveCatalog(db: D1Database): Promise<ActiveCommercialCatalog> {
  try {
    const version = await db.prepare(
      `SELECT version, checksum, published_at FROM catalog_versions WHERE status = 'active' LIMIT 1`,
    ...
    const [planResult, variantResult, creditResult] = await Promise.all([
      db.prepare(
        `SELECT plan_id, display_name, description, currency, recurring_amount, recurring_interval,
         ...
         FROM plan_catalog_entries WHERE catalog_version = ?1 AND active = 1
      ...
      db.prepare(
        `SELECT plan_id, billing_cycle, recurring_amount, recurring_interval, included_credits,
         ...
         FROM plan_billing_variants WHERE catalog_version = ?1 AND active = 1
      ...
      db.prepare(
        `SELECT product_id, display_name, description, currency, amount, credits, active, square_catalog_object_id
         FROM credit_products WHERE catalog_version = ?1 AND active = 1
    ...
    const plans = (planResult.results ?? []).map((row) => {
      ...
      const billingVariants = allVariants.filter((variant) => variant.plan_id === row.plan_id).map((variant) => ({
      ...
      return {
        plan_id: row.plan_id, id: row.plan_id, display_name: row.display_name, name: row.display_name,
        ...
        included_credits: Number(row.included_credits), monthly_credits: Number(row.included_credits),
        monthly_credits_label: Number(row.included_credits).toLocaleString('ja-JP'),
        entitlement_ids: entitlements, features: entitlements, recommended: row.recommended === 1,
        active: row.active === 1, square_plan_variation_id: monthly?.square_plan_variation_id ?? row.square_plan_variation_id,
        billing_variants: billingVariants,
      };
    });

    const creditProducts = (creditResult.results ?? []).map((row) => ({
      product_id: row.product_id, id: row.product_id, display_name: row.display_name, name: row.display_name,
      description: row.description, currency: row.currency, amount: Number(row.amount), price_label: yen(Number(row.amount)),
      credits: Number(row.credits), credits_label: Number(row.credits).toLocaleString('ja-JP'),
      active: row.active === 1, square_catalog_object_id: row.square_catalog_object_id,
    }));
    return { catalog_version: version.version, checksum: version.checksum, published_at: version.published_at, plans, creditProducts };
  } catch (error) {
