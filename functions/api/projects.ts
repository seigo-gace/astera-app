...
import { createProject,listProjects,WorkspaceStoreError } from '../_workspace-store';
...
export async function onRequestGet(c:C){const id=requestCorrelationId(c.request);try{return Response.json(await listProjects(c.env.ASTERA_DB,await actor(c)),{headers:{'Cache-Control':'no-store','X-Correlation-ID':id}});}catch(e){return functionErrorResponse(norm(e),id);}}
export async function onRequestPost(c:C){const id=requestCorrelationId(c.request);try{return Response.json(await createProject(c.env.ASTERA_DB,await actor(c),await c.request.json().catch(()=>null)),{status:201,headers:{'Cache-Control':'no-store','X-Correlation-ID':id}});}catch(e){return functionErrorResponse(norm(e),id);}}
