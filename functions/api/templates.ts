...
import { createTemplate, listTemplates, TemplateStoreError } from '../_template-store';
...
export async function onRequestGet(c: C) { const id=requestCorrelationId(c.request); try { return Response.json(await listTemplates(c.env.ASTERA_DB, await actor(c)), { headers:{'Cache-Control':'no-store','X-Correlation-ID':id} }); } catch(error) { return functionErrorResponse(norm(error), id); } }
export async function onRequestPost(c: C) { const id=requestCorrelationId(c.request); try { return Response.json(await createTemplate(c.env.ASTERA_DB, await actor(c), await c.request.json().catch(()=>null)), { status:201, headers:{'Cache-Control':'no-store','X-Correlation-ID':id} }); } catch(error) { return functionErrorResponse(norm(error), id); } }
