...
function parseBillingCycle(value: unknown): BillingCycle {
  ...
  throw new FunctionHttpError(
    ...
    "billing_cycleはmonthlyまたはannualを指定してください。",
  );
}
...
async function assertPlanCheckoutAvailable(
  ...
  const [subscription, pendingIntent] = await Promise.all([
    context.env.ASTERA_DB.prepare(
      `SELECT plan_id, billing_cycle, provider_subscription_id, status FROM tenant_subscriptions WHERE tenant_id = ?1 LIMIT 1`,
    )
      .bind(tenantId)
      .first<ExistingSubscription>(),
    context.env.ASTERA_DB.prepare(
      `SELECT id, status FROM billing_intents WHERE tenant_id = ?1 AND product_kind = 'plan' AND status IN ('creating_checkout','checkout_created','payment_pending','reconciliation_required') AND (expires_at IS NULL OR expires_at > ?2) ORDER BY created_at DESC LIMIT 1`,
    )
      .bind(tenantId, new Date().toISOString())
      .first<PendingPlanIntent>(),
  ]);
  if (pendingIntent)
    throw new FunctionHttpError(
      409,
      ...
      { intent_id: pendingIntent.id, status: pendingIntent.status },
    );
  const live =
...
export async function onRequestPost(context: PagesContext): Promise<Response> {
  ...
  try {
    ...
    if (!body)
      throw new FunctionHttpError(
        400,
        "CHECKOUT_BODY_INVALID",
        "Checkout RequestのJSONを確認できません。",
      );
    ...
    if (Boolean(productId) === Boolean(planId))
      throw new FunctionHttpError(
        422,
        "CHECKOUT_PRODUCT_SELECTION_INVALID",
        "product_idまたはplan_idのどちらか一つを指定してください。",
      );
    const billingCycle = planId ? parseBillingCycle(body.billing_cycle) : null;

    const existing = await context.env.ASTERA_DB.prepare(
      `SELECT id, tenant_id, user_id, status, checkout_url, provider_checkout_id, provider_order_id, expires_at FROM billing_intents WHERE idempotency_key = ?1 LIMIT 1`,
    )
      .bind(key)
      .first<ExistingIntent>();
    if (existing) {
      if (
        existing.tenant_id !== actor.profile.tenant_id ||
        existing.user_id !== actor.user.id
      )
        throw new FunctionHttpError(
          409,
          "IDEMPOTENCY_KEY_OWNERSHIP_MISMATCH",
          "このIdempotency-Keyは別Contextで使用されています。",
        );
      ...
      throw new FunctionHttpError(
        409,
        "CHECKOUT_INTENT_IN_PROGRESS",
        "同じCheckout Intentを作成中です。",
      );
    }
    if (planId && billingCycle)
      await assertPlanCheckoutAvailable(
        context,
    ...
    if (productId) {
      const product = catalog.creditProducts.find(
        (item) => item.product_id === productId && item.active,
      );
      if (!product)
        throw new FunctionHttpError(
          422,
          "CREDIT_PRODUCT_NOT_AVAILABLE",
          "Active Catalogに存在するCredit商品を選択してください。",
        );
      productKind = "credit";
      ...
      const plan = catalog.plans.find(
        (item) => item.plan_id === planId && item.active,
      );
      if (!plan || !billingCycle)
        throw new FunctionHttpError(
          422,
          "PLAN_NOT_AVAILABLE",
          "Accountが選択可能なPlanではありません。",
        );
      const variant = plan.billing_variants.find(
        (item) => item.billing_cycle === billingCycle && item.active,
      );
      if (!variant)
        throw new FunctionHttpError(
          422,
          "PLAN_BILLING_VARIANT_NOT_AVAILABLE",
          "選択したPlanの請求周期は利用できません。",
        );
      if (variant.recurring_amount <= 0)
        throw new FunctionHttpError(
          422,
          "PLAN_CHECKOUT_NOT_REQUIRED",
          "このPlanはSquare Checkoutを必要としません。",
        );
      if (!variant.square_plan_variation_id)
        throw new FunctionHttpError(
          503,
          "SQUARE_PLAN_MAPPING_MISSING",
          "PlanとSquare Subscription VariationのMappingがありません。",
    ...
    if (
      ...
      throw new FunctionHttpError(
        500,
        "CATALOG_MONEY_CONTRACT_INVALID",
        "Catalogの金額またはCredit量が不正です。",
      );
    ...
    await context.env.ASTERA_DB.batch([
      context.env.ASTERA_DB.prepare(
        `INSERT INTO return_contexts (id, tenant_id, user_id, route, reference_type, reference_id, private_mode, resume_mode, expires_at, consumed_at, created_at) VALUES (?1, ?2, ?3, ?4, 'billing_intent', ?5, 0, 'user_confirm', ?6, NULL, ?7)`,
      ).bind(
        contextId,
        actor.profile.tenant_id,
        actor.user.id,
      ...
      context.env.ASTERA_DB.prepare(
        `INSERT INTO billing_intents (id, tenant_id, user_id, catalog_version, product_id, currency, amount, status, idempotency_key, provider_checkout_id, created_at, updated_at, product_kind, billing_cycle, credit_amount, provider_order_id, provider_payment_id, checkout_url, return_context_id, expires_at, completed_at, failure_code) VALUES (?1, ?2, ?3, ?4, ?5, 'JPY', ?6, 'creating_checkout', ?7, NULL, ?8, ?8, ?9, ?10, ?11, NULL, NULL, NULL, ?12, ?13, NULL, NULL)`,
      ).bind(
        intentId,
        actor.profile.tenant_id,
        actor.user.id,
    ...
    try {
      ...
      await context.env.ASTERA_DB.prepare(
        `UPDATE billing_intents SET status = 'checkout_created', provider_checkout_id = ?1, provider_order_id = ?2, checkout_url = ?3, updated_at = ?4 WHERE id = ?5 AND status = 'creating_checkout'`,
      )
        .bind(
          square.checkoutId,
          square.orderId,
      ...
      const code =
        error instanceof FunctionHttpError
          ? error.code
          : "SQUARE_CHECKOUT_CREATE_FAILED";
      await context.env.ASTERA_DB.prepare(
        `UPDATE billing_intents SET status = 'failed', failure_code = ?1, updated_at = ?2 WHERE id = ?3`,
      )
        .bind(code, new Date().toISOString(), intentId)
        .run();
      throw error;
    }
  } catch (error) {
