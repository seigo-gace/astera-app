...
async function updateEvent(
  ...
  await env.ASTERA_DB.prepare(
    `UPDATE billing_events
     SET billing_intent_id = ?1, processing_status = ?2, processed_at = ?3
...
async function handlePaymentEvent(env: Env, event: SquareEvent): Promise<string> {
  ...
  if (paidAmount === null || paidAmount !== Number(intent.amount) || paidCurrency !== intent.currency) {
    await env.ASTERA_DB.batch([
      ...
      env.ASTERA_DB.prepare(
        `UPDATE billing_events SET billing_intent_id = ?1, processing_status = 'reconciliation_required', processed_at = ?2
  ...
  if (status === 'COMPLETED') {
      ...
      await env.ASTERA_DB.batch([
          ...
          `UPDATE credit_accounts
           ...
           WHERE id = ?3
             AND NOT EXISTS (
               ...
               WHERE reference_type = 'billing_intent' AND reference_id = ?4 AND kind = 'grant'
             )`,
        ...
        env.ASTERA_DB.prepare(
          `INSERT OR IGNORE INTO credit_ledger
           ...
           VALUES (?1, ?2, 'grant', ?3, ?4, 'billing_intent', ?5, ?6, ?7)`,
        ).bind(transactionId, creditAccount.id, Number(intent.credit_amount), `square:${eventId}:grant`, intent.id, fingerprint, now),
        env.ASTERA_DB.prepare(
          `UPDATE billing_intents
        ...
        env.ASTERA_DB.prepare(
          `UPDATE billing_events
           SET billing_intent_id = ?1, processing_status = 'processed', processed_at = ?2
           WHERE provider_event_id = ?3`,
    ...
    await env.ASTERA_DB.batch([
      env.ASTERA_DB.prepare(
        `UPDATE billing_intents
         SET status = 'reconciliation_required', provider_payment_id = ?1,
             failure_code = 'SUBSCRIPTION_ID_RECONCILIATION_REQUIRED', updated_at = ?2
         WHERE id = ?3`,
      ).bind(paymentId || null, new Date().toISOString(), intent.id),
      env.ASTERA_DB.prepare(
        `UPDATE billing_events
         SET billing_intent_id = ?1, processing_status = 'reconciliation_required', processed_at = ?2
         WHERE provider_event_id = ?3`,
      ).bind(intent.id, new Date().toISOString(), eventId),
    ]);
    return 'reconciliation_required';
    ...
    env.ASTERA_DB.prepare(
      `UPDATE tenant_subscriptions
       SET status = ?1, current_period_start = COALESCE(?2, current_period_start),
           current_period_end = COALESCE(?3, current_period_end), updated_at = ?4
       WHERE id = ?5`,
    ).bind(
      normalizedStatus,
...
export async function onRequestPost(context: PagesContext): Promise<Response> {
  ...
  try {
    ...
    ).bind(eventId).first<{ processing_status: string }>();
    if (existing?.processing_status && existing.processing_status !== 'processing') {
      return Response.json({ accepted: true, duplicate: true, event_id: eventId, processing_status: existing.processing_status }, {
    ...
    await context.env.ASTERA_DB.prepare(
      `INSERT OR IGNORE INTO billing_events
        (provider_event_id, billing_intent_id, signature_verified, event_type, received_at, processed_at, processing_status)
       VALUES (?1, NULL, 1, ?2, ?3, NULL, 'processing')`,
    ...
    return Response.json({ accepted: true, event_id: eventId, processing_status: processingStatus }, {
      headers: { 'Cache-Control': 'no-store', 'X-Correlation-ID': requestId },
    });
  } catch (error) {
