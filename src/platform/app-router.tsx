...
function currentReturnTo(): string { return window.location.pathname + window.location.search + window.location.hash; }
function accountProjection(payload: unknown) { const root = asRecord(payload); return asRecord(root.account ?? root.data ?? root); }
...
function AccountSessionGate({ children }: { children: ReactNode }) {
  if (previewWithoutAuth()) return <AccountSessionProvider value={PREVIEW_ACCOUNT_SESSION}>{children}</AccountSessionProvider>;
...
function AccountSessionGateLive({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    apiRequest('/api/account', { signal: controller.signal }).then((payload) => {
      const returnTo = currentReturnTo(); const continuation = accountContinuation(payload, returnTo);
      if (continuation) { window.location.replace(continuation); return; }
      const account = accountProjection(payload); const accountStatus = recordText(account, ['account_status', 'status']);
      if (accountStatus && accountStatus !== 'active') { setState({ status: 'error', error: new ApiError('Accountの現在状態ではこのPageを利用できません。', 403, `ACCOUNT_${accountStatus.toUpperCase()}`, payload) }); return; }
      ...
      if (error instanceof ApiError) {
        ...
        if (error.status === 401 || (error.status === 403 && authenticationCodes.has(error.code))) { const returnTo = encodeURIComponent(currentReturnTo()); window.location.replace(`/login?return_to=${returnTo}`); return; }
      }
...
function RootRedirect() { useEffect(() => { window.location.replace('/app/new'); }, []); return <BusyState label="Astera Appを開いています…" />; }
function LegacySearchRedirect() { useEffect(() => { window.location.replace('/app/history'); }, []); return null; }
...
export default function AppRouter() {
  ...
  if (route.id === 'account-checkout') return <AccountSessionGate><CheckoutPage route={route} /></AccountSessionGate>;
  if (route.id === 'app' || route.id === 'new-run') return <AccountSessionGate><NativeComposerPage route={route} /></AccountSessionGate>;
