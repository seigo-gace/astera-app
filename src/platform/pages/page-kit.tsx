...
export function safeNavigate(path: string): void {
  const target = safeReturnPath(path, '/app/new');
  ...
  return (
    <label className="platform-field">
      ...
      <input
        ...
        inputMode={inputMode}
        value={value}
        onChange={onChange ? (event: { target: HTMLInputElement }) => onChange(event.target.value) : undefined}
      />
    </label>
  );
}
...
export function SelectField({
  ...
  return (
    <label className="platform-field">
      <span>{label}</span>
      <select name={name} value={value} onChange={(event: { target: HTMLInputElement }) => onChange(event.target.value)}>
...
export function FormResult({ state }: { state: { type: 'idle' | 'working' | 'success' | 'error'; message?: string; code?: string } }) {
  ...
  return (
    <div className={`platform-form-result is-${state.type}`} role={state.type === 'error' ? 'alert' : 'status'}>
...
export async function submitForm(
  ...
  options: { method?: 'POST' | 'PATCH' | 'DELETE'; success?: string; navigateTo?: string; idempotent?: boolean } = {},
  ...
  return (
    <dl className="platform-kv-grid">
      {entries.map(([key, item]) => (
        <div key={key}><dt>{key.replace(/_/g, ' ')}</dt><dd>{textValue(item, '—')}</dd></div>
      ))}
    </dl>
  );
}
...
export function RecordList({
  items,
  titleKeys = ['title', 'name', 'display_name', 'id'],
  subtitleKeys = ['description', 'summary', 'status', 'created_at'],
  link,
  empty = '対象Dataはありません。',
}: {
  items: unknown[];
  ...
  return (
    <div className="platform-record-list">
      {items.map((item, index) => {
        ...
        return href
          ? <a className="platform-record" href={href} key={`${title}-${index}`}>{content}<b aria-hidden="true">›</b></a>
          : <div className="platform-record" key={`${title}-${index}`}>{content}</div>;
      })}
    </div>
  );
}
...
export function ResourceShell({
  ...
  return (
    <ResponsivePageShell route={route} description={description}>
      {state.status === 'loading' && <BusyState />}
      {state.status === 'error' && <ErrorState error={state.error} onRetry={reload} />}
      {state.status === 'ready' && children(state.data, reload)}
    </ResponsivePageShell>
  );
}
