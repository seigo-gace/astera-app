...
function CreditMeter({ enabled }: { enabled: boolean }) {
  ...
  return <div className="platform-credit-layer" aria-live="polite"><a href="/app/plan-credit" className={`platform-credit-meter is-${credit.state}`} style={meterStyle} role="meter" aria-label={`${text('usableCredit')} ${creditLabel}`} aria-valuemin={0} aria-valuemax={credit.capacity} aria-valuenow={credit.usable} title={credit.reserved > 0 ? `${text('reservedCredit')} ${credit.reserved.toLocaleString(locale)} ©` : `${text('usableCredit')} ${creditLabel}`}><span className="platform-credit-mark" aria-hidden="true" /><span className="platform-credit-title">CREDIT</span><span className="platform-credit-track" aria-hidden="true"><span className="platform-credit-fill" /></span><strong className="platform-credit-number">{creditLabel}</strong></a></div>;
}
