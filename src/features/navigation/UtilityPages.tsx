...
import './plan-credit-storage.css';
...
function storagePackFallbacks(language: 'ja' | 'en'): StoragePack[] {
  const plus = language === 'ja' ? '＋' : '+';
  return [
    { productId: 'storage_1gb', displayName: `${plus}1GB`, capacityGb: 1, priceJpy: 480, canPurchase: false },
    { productId: 'storage_10gb', displayName: `${plus}10GB`, capacityGb: 10, priceJpy: 1980, canPurchase: false },
    { productId: 'storage_50gb', displayName: `${plus}50GB`, capacityGb: 50, priceJpy: 5980, canPurchase: false },
  ];
}
...
function numberFromLabel(value: string | undefined): number {
  if (!value) return 0;
    ...
    <section className="plan-credit-section">
      <div className="plan-credit-section-head">
        ...
        <div className="plan-credit-cycle-toggle" role="group" aria-label={`${monthlyLabel} / ${annualLabel}`}>
          <button type="button" className={billingCycle === 'monthly' ? 'is-active' : ''} aria-pressed={billingCycle === 'monthly'} onClick={() => onBillingCycleChange('monthly')}>{monthlyLabel}</button>
          <button type="button" className={billingCycle === 'annual' ? 'is-active' : ''} aria-pressed={billingCycle === 'annual'} onClick={() => onBillingCycleChange('annual')}>{annualLabel}</button>
        </div>
      </div>
      <div className="plan-credit-grid">
        {items.map((item) => {
          ...
          const isCurrentPlan = Boolean(subscription.planId) && itemPlanId === subscription.planId && (isFree || subscription.billingCycle === billingCycle);
          const usesSubscriptionManagement = subscription.hasLiveSubscription;
          ...
          const price = billingCycle === 'annual' ? (item.annualPrice ?? item.monthlyPrice ?? item.price) : (item.monthlyPrice ?? item.price);
          ...
          const cardContent: ReactNode = (
            <>
                ...
                <div className="plan-credit-plan-heading">
                  ...
                  {billingCycle === 'annual' && !isFree && (
                    <div className="plan-credit-annual-meta">
                      ...
                      {item.annualMonthlyEquivalent && <span>{monthlyEquivalent}: {item.annualMonthlyEquivalent}</span>}
                    </div>
                  )}
                </div>
                {isCurrentPlan && (
              ...
              {item.creditValue && (
                <div className="plan-credit-fact"><span>{item.creditLabel ?? creditLabel}</span><strong>{item.creditValue}</strong></div>
              )}
              {billingCycle === 'annual' && !isFree && <div className="plan-credit-monthly-grant">{monthlyGrant}</div>}
              {hasFeatureContent && (
                <>
                      ...
                      <div className="plan-credit-basic-columns">
                        {item.basicFeature.columns.map((column, columnIndex) => (
                          <div className="plan-credit-basic-column" key={`${item.name}-basic-${columnIndex}`}>
                            ...
                            <ul className="plan-credit-basic-list">{column.items.map((value) => <li key={value}>{value}</li>)}</ul>
                          </div>
                        ))}
                      </div>
                    </div>
                  )}
                  {item.features && item.features.length > 0 && <ul className="plan-credit-feature-list">{item.features.map((feature) => <li key={feature}>{feature}</li>)}</ul>}
                </>
              )}
            </>
          );
          ...
          if (isFree || isCurrentPlan) {
            return <article className={`plan-credit-card is-plan${isCurrentPlan ? ' is-current-plan' : ''}`} key={item.name}>{cardContent}</article>;
          }
          return <a className="plan-credit-card is-plan is-actionable" href={actionHref} key={item.name} aria-label={`${item.name}・${cycleLabel}`}>{cardContent}</a>;
        })}
      </div>
    </section>
  );
...
function CreditGrid({ title, items, defaultCreditLabel }: {
  title: string;
  items: readonly PlanCreditCard[];
  ...
  return (
    <section className="plan-credit-section">
      ...
      <div className="plan-credit-grid">
        {items.map((item) => {
          const amount = numberFromLabel(item.price);
          const credits = numberFromLabel(item.creditValue);
          const href = `/account/checkout?kind=credit&amount=${amount}&credits=${credits}&return_to=plan-credit`;
          return (
            <a className="plan-credit-card is-credit is-actionable" href={href} key={item.name}>
              <h3>{item.name}</h3>
              {item.price && <div className="plan-credit-price">{item.price}</div>}
              {item.creditValue && <div className="plan-credit-fact"><span>{item.creditLabel ?? defaultCreditLabel}</span><strong>{item.creditValue}</strong></div>}
            </a>
          );
        })}
      </div>
...
function StorageSection({ language, previewMode }: { language: 'ja' | 'en'; previewMode: boolean }) {
  ...
  const reload = () => {
    if (previewMode) {
      setLoad({ status: 'ready', data: {
        planId: 'free', planMaxCapacityGb: 1, currentCapacityGb: 0, remainingPurchaseCapacityGb: 1,
        usedBytes: 0, remainingBytes: 0, state: 'inactive', writeAllowed: false, overPlanLimit: false, packs: fallbackPacks,
      } });
      return;
    }
    ...
    apiRequest('/api/storage/catalog')
      ...
      .catch((error: unknown) => setLoad({ status: 'error', message: error instanceof Error ? error.message : copy.loadError }));
  };
  ...
  return (
    <section className="plan-credit-section plan-credit-storage-section">
      ...
      {load.status === 'loading' && <div className="plan-credit-storage-status">{copy.loading}</div>}
      {load.status === 'error' && (
        <div className="plan-credit-storage-status is-error"><span>{load.message}</span><button type="button" onClick={reload}>再試行</button></div>
      )}
      {load.status === 'ready' && (
        <>
          <div className="plan-credit-storage-overview">
            ...
            <div className="plan-credit-storage-gauge" role="progressbar" aria-label={`${copy.usage} ${formatBytes(load.data.usedBytes)}`} aria-valuemin={0} aria-valuemax={Math.max(1, load.data.currentCapacityGb * 1024 ** 3)} aria-valuenow={Math.min(load.data.usedBytes, Math.max(1, load.data.currentCapacityGb * 1024 ** 3))}>
...
export function PlanCreditPage({ route }: { route: RouteMatch }) {
  ...
  useEffect(() => {
    ...
    apiRequest('/api/account/catalog', { signal: controller.signal })
      ...
      .catch(() => {
        // Keep the page usable when current-plan readback is temporarily unavailable.
      });
  ...
  return (
    <ResponsivePageShell route={route} fullWidth>
      <div className="plan-credit-page">
        <header className="plan-credit-local-head"><h1>{pageText.pageTitle}</h1></header>
        <PlanGrid
        ...
        />
        <CreditGrid title={pageText.creditSectionTitle} items={pageText.credits} defaultCreditLabel={pageText.grantedCredit} />
