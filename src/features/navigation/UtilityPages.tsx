...
import {
  previewWithoutAuth,
  useVerifiedAccountSession,
} from "../../platform/account-session";
...
type FeatureGroup = { title?: string; items: readonly string[] };
type PlanCreditCard = {
  id?: string;
  name: string;
  price?: string;
  ...
  basicFeature?: { label: string; columns: readonly FeatureGroup[] };
};
...
function subscriptionFromPayload(payload: unknown): SubscriptionProjection {
  ...
  const planId = normalizePlanId(
    recordText(subscription, ["plan_id", "planId"]),
  );
  const providerSubscriptionId = recordText(subscription, [
    "provider_subscription_id",
    "providerSubscriptionId",
  ]);
  const status = recordText(subscription, ["status"]).toLowerCase();
  const billingCycle = normalizeBillingCycle(
    recordText(subscription, ["billing_cycle", "billingCycle"]),
  );
  return {
    planId,
    billingCycle,
    hasLiveSubscription:
      ...
      !["none", "cancelled", "failed"].includes(status),
  };
}
  ...
  return (
    <section className="plan-credit-section">
      <div className="plan-credit-section-head">
        <h2>{title}</h2>
        <div
          className="plan-credit-cycle-toggle"
          role="group"
      ...
      <div className="plan-credit-grid">
        {items.map((item) => {
          const hasFeatureContent = Boolean(
            item.basicFeature || (item.features && item.features.length > 0),
          );
          ...
          const usesSubscriptionManagement =
            subscription.hasLiveSubscription || isFree;
          const actionLabel = usesSubscriptionManagement
            ? changePlanLabel
            : selectPlanLabel;
          ...
          const content: ReactNode = (
            <>
              <div className="plan-credit-plan-top">
                <div className="plan-credit-plan-heading">
                  ...
                  {billingCycle === "annual" && !isFree && (
                    <div className="plan-credit-annual-meta">
                      <span className="plan-credit-saving-badge">
                        {annualSaving}
                      </span>
                      {item.annualMonthlyEquivalent && (
                        <span>
                          {monthlyEquivalent}: {item.annualMonthlyEquivalent}
                        </span>
                      )}
                    </div>
                  )}
                ...
                {isCurrentPlan && (
                  <div
                    className="plan-credit-current-plan"
                    aria-label={selectedPlanLabel}
                  >
                    <span
                      className="plan-credit-current-plan-icon"
                      aria-hidden="true"
                    />
                    <span>
                      {isFree
                        ? selectedPlanLabel
                        : `${selectedPlanLabel}・${cycleLabel}`}
                    </span>
                  </div>
                )}
              </div>
              ...
              {hasFeatureContent && (
                <>
                  <div className="plan-credit-feature-title">
                    {featureLabel}
                  </div>
                  {item.basicFeature && (
                    <div className="plan-credit-basic-feature">
                      <div className="plan-credit-basic-label">
                        {item.basicFeature.label}
                      </div>
                      <div className="plan-credit-basic-columns">
                        {item.basicFeature.columns.map((column, index) => (
                          <div
                            className="plan-credit-basic-column"
                            key={`${item.name}-${index}`}
                          >
                            {column.title && (
                              <div className="plan-credit-basic-column-title">
                                {column.title}
                              </div>
                            )}
          ...
          return isCurrentPlan ? (
            <article
              className="plan-credit-card is-plan is-current-plan"
              key={item.name}
            >
              {content}
            </article>
  ...
  return (
    <section className="plan-credit-section">
      ...
      <div className="plan-credit-grid">
        {items.map((item) => (
          <article
            className={`plan-credit-card${item.price ? " is-credit" : " is-storage"}`}
            key={item.name}
          >
            <h3>{item.name}</h3>
            {item.price && (
