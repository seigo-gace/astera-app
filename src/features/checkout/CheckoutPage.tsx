...
function billingCycle(value: string | null): BillingCycle {
...
export default function CheckoutPage({ route }: { route: RouteMatch }) {
  ...
  const cycle = billingCycle(params.get("billing"));
  const selectedPlan =
    planText.plans.find((plan) => plan.id === planId) ?? null;
            ...
            >
              ...
              {connection.status === "error" && (
                <>
                  ...
                  <button
                    type="button"
                    onClick={() => void loadAccountCatalog()}
                  >
                    {text.retry}
                  </button>
                </>
